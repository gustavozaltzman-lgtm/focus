...
| backend | `npm run import-fichas -- "<ruta.txt>" [email]` | Importa "fichas" (texto estructurado `FICHA N`/`FICHA #N` / `Título` / `Descripción` / `Fecha` / `Contexto` / `Estado` / `Prioridad` / `Fuente`) a Inbox. Idempotente: correrlo de nuevo sobre el mismo archivo no duplica lo ya importado, solo agrega fichas nuevas. Ver `backend/src/scripts/import-fichas.ts`. |
